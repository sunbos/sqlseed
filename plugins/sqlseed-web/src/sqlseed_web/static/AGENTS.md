...
上层见 [后端指南](../AGENTS.md)。本目录由 FastAPI 直接提供，使用原生 ES modules、DOM 与 CSS，无 bundler、CDN 或 npm 构建。

## 当前产品契约

- v8 整体重建已完成，当前以 [可用性与 AI 辅助计划](../../../../../docs/superpowers/plans/2026-09-07-workbench-usability-ai.md) 为细化依据；其后依次是 [v8 重建契约](../../../../../docs/superpowers/plans/2026-09-07-web-v8-rebuild.md)、v8 原型 README/HTML 和重设计提案未被后续评审覆盖的业务规则。保留 v8 单一主题与当前四项导航；本轮浏览器、真实 LLM 与 PostgreSQL 验收分别记录，不能把计划或自动测试当作全部验收通过。
...
- 从 v8 重建整个 shell、叶芽表格 SVG logo、字体、色彩与空间节奏；只有一套生效主题。`index.html` 仅加载 `style.css`，后者导入组件样式并承载 v8 基础规则；`workbench.css` 只补组件类，不定义第二套 tokens、body 或旧 Material 3 覆盖。
...
| [js/pages/](js/pages/) | 正式 workbench/runs；其余页面为历史模块，不可由 router 加载 |
...
- 主导航为工作台、运行记录、配置管理和设置；不可因旧页面或 API 仍存在而增加产品入口。用户后续评审已授权工作台内的 AI 辅助面板，仍不恢复旧 heal、诊断或历史页面。
...
## 历史 genform 面板回归（不作为新工作台布局约束）

本节记录旧 `genform.js` 的行为，只有维护历史模块时适用。数据库硬约束、配置无损往返和参数单位仍需在新实现保持；旧面板顺序、默认交互、分类占位与能力裁剪不得覆盖当前契约。
...
本节仅约束保留的 legacy 模块与原有 API 集成；不要求新产品显示这些页面，不代表新工作台的执行边界。
...
## 标准化交互（2026-09-07 后续评审）

- 当前实施依据为 [标准化实施计划](../../../../../docs/superpowers/plans/2026-09-07-workbench-standards-implementation.md)。配置管理的第三项导航是用户后续授权，不恢复旧产品页面。
...
- 最新对象上下文细化见 [当前表与业务夹具计划](../../../../../docs/superpowers/plans/2026-09-07-workbench-context-and-scenario.md) 与 [阶段二评审实施计划](../../../../../docs/superpowers/plans/2026-09-08-workbench-review-improvements.md)。配置工具与引擎/语言合并为紧凑上下文区，保留直接可见的全局设置、保存状态及生成主入口；侧栏表名与生成/引用状态分行，勾选、字段和依赖路径入口独立，完整名称可获取。AI 使用同一“AI 配置助手”，顶栏保留固定入口，后续批准的使用引导可提供范围快捷入口；NOT NULL 不渲染 NULL 控件，可空百分比按开关显示。侧栏展开状态按 model 保留，查找表不改勾选；内容按自身高度收缩，长表有限滚动且固定表头。
...
- 用户后续批准首次使用引导：`guidance.js` 只根据现有配置、输入问题和当前 epoch 的完整预览计算下一步，不追认业务规则已审阅。`workbench.js` 复用现有操作门禁和写入确认；引导内 AI 快捷操作与顶栏打开同一助手，默认已选表，不增加独立 AI 流程或自动分析。收起偏好属于浏览器，不改变生成文档；缺插件/待配置/配置已填写仅来自脱敏配置响应，不能当作连通性检测。
...
- 本轮验收见 [十项反馈实施记录](../../../../../docs/design-review/2026-09-10-ui-review-verification.md)。

## 2026-09-11 插件维护（兼容显式维护启动）
...
## 2026-09-11 网页内组件管理与预览收敛

- 当前修正依据为 [网页内组件管理与预览设计](../../../../../docs/superpowers/specs/2026-09-11-web-native-plugin-and-preview-design.md)。默认受管启动由 `automatic_lifecycle` 表示；`enabled` 不等于旧维护模式，不得因安装能力可用而禁用 AI 设置或锁死导航。
