...
let current = { connId: null, table: null };
...
async function loadLeft() {
  ...
  try {
    ...
    if (isCurrent()) { clear(left); left.append(msg(error.message)); }
    return;
  }
  if (!isCurrent()) return;
  clear(left);
  ...
  for (const c of conns.connections) {
    ...
    const tablesNode = h('div', {}, h('div', { class: 'loading' }, '加载表…'));
  ...
  await Promise.all(conns.connections.map(async (c) => {
    try {
      ...
      if (isCurrent()) { clear(c.tablesNode); c.tablesNode.append(msg(error.message)); }
    }
  }));
}
...
async function selectTable(connId, tableName) {
  ...
  try {
    const [detail, schema, mapping, rows] = await Promise.all([
    ...
    if (!isCurrent()) return;
    current = { connId, table: tableName };
    store.connId = connId;
    ...
    if (isCurrent()) { clear(center); center.append(msg(error.message)); }
  }
}
...
function renderCenter(center, connId, tableName, rowsRes) {
  ...
  center.append(
    h('div', { class: 'row' },
      ...
      h('span', { style: 'flex:1' }),
      h('button', { class: 'small', onclick: () => selectTable(connId, tableName) }, '刷新'),
    ),
...
function renderRight(right, tableName, schema, mapping) {
  ...
  right.append(
    ...
    ...((schema.foreign_keys || []).length ? [h('h3', { class: 'section-title' }, '外键')] : []),
