...
export function openDataPreview({tables,currentTable,selectedTables,initialScope='current',initialCount=10,
  generate,guard=task=>task,isCurrent=()=>true,onOptionsChange,onResult,onError,onColumnAction,container=null,fixedScope=false,
  initialResult=null,initialView=null,initialStale=false}) {
  ...
  let count=validCount(initialView?.count)?Number(initialView.count):validCount(initialCount)?Number(initialCount):10;
  ...
  const status=h('p',{class:'wb-preview-status',role:'status','aria-live':'polite'},'点击“生成预览”查看当前规则的效果。');
  const error=h('p',{id:`${id}-error`,class:'wb-preview-error',role:'alert'});
  ...
  const countInput=h('input',{type:'number',min:1,max:100,step:1,value:String(count),
    'aria-label':'每表预览行数','data-db-action':'','aria-describedby':`${id}-count-help ${id}-error`});
  const countField=h('label',{class:'wb-preview-field'},h('span',{},'每表预览行数'),countInput);
  const controls=h('div',{class:'wb-preview-controls'},...(fixedScope?[]:[scopeHolder]),countField);

  function live() {return !closed && dialog.el.isConnected && isCurrent();}
  function renderScope() {
    scopeControl?.destroy();
    if(fixedScope)return;
    scopeControl=createDropdown({label:'预览范围',value:scope,options:[
      {value:'current',label:`当前表 · ${currentTable || '未选择'}`},
      {value:'selected',label:`已选表 · ${selected.length} 张`},
    ],onChange:value=>{
      if(closed || busy)return;
      scope=value;shownTable=scope==='current'?currentTable:selected[0];selectedColumn=null;optionsChanged();
    }});
  ...
  function countValid() {
    ...
    count=Number(countInput.value);return true;
  }
  ...
  function setBusy(value) {
    busy=value;countInput.disabled=value;refreshButton.disabled=value;
    controls.setAttribute('aria-busy',String(value));results.setAttribute('aria-busy',String(value));
    for(const tab of tabs.querySelectorAll('button'))tab.disabled=value;
    for(const input of results.querySelectorAll('[data-preview-column]'))input.disabled=value;
    if(value) {
      ...
      scopeControl?.destroy();if(scopeControl)scopeControl.el.querySelector('.dropdown-btn').disabled=true;
    } else renderScope();
  }
  function cellText(table,column,row) {
    if(Object.prototype.hasOwnProperty.call(row,column.name))return {text:valueText(row[column.name]),placeholder:false};
    if(table?.omittedColumns?.[column.name])return {text:table.omittedColumns[column.name],placeholder:true};
    if(column.is_autoincrement || column.is_rowid_alias)return {text:'数据库分配',placeholder:true};
    if(column.is_computed)return {text:'数据库计算',placeholder:true};
  ...
  function columnHeader(table,column) {
    ...
    if(column.is_primary_key || table?.primary_key?.includes(column.name))metadata.push('PK');
    if(table?.foreign_keys?.some(key=>key.columns?.includes(column.name)))metadata.push('FK');
    if(typeof column.nullable==='boolean')metadata.push(column.nullable?'允许 NULL':'NOT NULL');
    const label=h('span',{class:'wb-preview-column-name'},column.name),details=h('small',{class:'wb-preview-column-meta'},metadata.filter(Boolean).join(' · '));
    if(!onColumnAction)return h('th',{scope:'col'},label,details);
    const activate=action=>()=>{
      if(!live() || busy)return;
      selectedColumn=column.name;columnAction=action;
      for(const input of results.querySelectorAll('[data-preview-column]'))input.setAttribute('aria-pressed',String(input.getAttribute('data-preview-column')===selectedColumn));
      return onColumnAction(action,{table:shownTable,column:column.name,view:getView()});
    };
    const attributes=action=>({plain:true,class:`wb-preview-column-button wb-preview-column-${action}`,
      'data-preview-column':column.name,'data-preview-entry':action,'aria-pressed':String(column.name===selectedColumn),
      'aria-haspopup':'dialog',disabled:busy});
    const name=button('',activate('information'),{...attributes('information'),
      'aria-label':`查看 ${shownTable}.${column.name} 的字段信息`,title:'查看字段信息'});
    name.append(label);
    const rule=table?.ruleSummaries?.[column.name];
    const properties=button(`规则：${rule?.label || '取值规则'}`,activate('rule'),{...attributes('rule'),
      'aria-label':`调整 ${shownTable}.${column.name} 的生成规则`,title:rule?.detail || '调整生成规则'});
    details.title='数据库结构';
    return h('th',{scope:'col',class:'wb-preview-interactive-header'},name,details,properties);
  }
  function renderRows() {
    if(!result)return;
    ...
    const summary=h('p',{class:'wb-preview-summary'},`${name} · 实际展示 ${rows.length} 行 · 每表最多 ${count} 行`);
    if(!rows.length) {
    ...
    const columns=table?.columns?.length?table.columns:[...new Set(rows.flatMap(row=>Object.keys(row)))].map(name=>({name}));
    const data=h('table',{class:'wb-preview-data'},h('caption',{},`${name} · 预览记录`),
      h('thead',{},h('tr',{},h('th',{scope:'col'},'#'),...columns.map(column=>columnHeader(table,column)))),
      h('tbody',{},...rows.map((row,index)=>h('tr',{},h('th',{scope:'row'},String(index+1)),...columns.map(column=>{
        const value=cellText(table,column,row);
        return h('td',{class:value.placeholder?'wb-preview-placeholder':''},value.text);
      })))));
  ...
  function renderResult() {
    ...
    tabs.replaceChildren(...(scope==='selected'?names.map(name=>button(name,()=>{
      if(closed || busy)return;
      rememberTableScroll();
      shownTable=name;selectedColumn=null;
      for(const item of tabs.querySelectorAll('button'))item.setAttribute('aria-pressed',String(item.textContent===name));
      renderRows();
    },{small:true,class:'wb-preview-table-button','aria-pressed':String(name===shownTable)})):[]));
    issues.replaceChildren(...(result.issues?.length?[h('ul',{},...result.issues.map(issue=>h('li',{
  ...
  const refresh=guard(async()=>{
    if(closed || busy)return;
    error.textContent='';
    if(!live()) {error.textContent='配置已变化，请重新打开预览。';return;}
    if(!countValid())return;
    if(scope==='selected' && !selected.length) {error.textContent='请先勾选需要预览的表，或切换到当前表。';return;}
    ...
    try {
      let response;
      try {response=await generate(options);} catch(failure) {
        if(stillCurrent()) {
          error.textContent=failure?.message || String(failure);
          status.textContent=result?(stale?'更新失败；规则已改变，当前为旧样例，请重新预览。':'更新失败，当前为上次结果，可重新预览。'):'预览未完成，可重试。';
          onError?.(failure);
        }
        return;
      }
      ...
      if(!response) {
        ...
        status.textContent=result?'当前为上次结果，请重新打开预览。':'';return;
      }
      result=response;stale=false;renderResult();refreshButton.textContent='重新预览';
      status.textContent=response.ok===false?'预览存在待处理项，请查看下方提示。'
        :response.preview_complete===false?'已展示可预览数据；部分关联记录需等待依赖数据就绪。':'预览数据已更新，数据库未写入。';
      onResult?.(response,options);
      return response;
    } finally {
      if(!closed && dialog.el.isConnected) {
        if(!result)results.replaceChildren();
        setBusy(false);
        if(!isCurrent()) {
  ...
  const refreshButton=button(container?'生成预览':'重新预览',refresh,{glyph:'refresh','data-db-action':''});
  controls.append(refreshButton);
  renderScope();
  dialog.body.append(help,controls,h('p',{id:`${id}-count-help`,class:'wb-preview-help'},
