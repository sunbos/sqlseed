...
const populated=value=>Array.isArray(value)?value.length>0:value&&typeof value==='object'?Object.keys(value).length>0:Boolean(value);

export function fieldAIEligibility(table,column,rule) {
  const denied=(code,reason)=>({eligible:false,code,reason});
  ...
  return {eligible:true,code:'editable',reason:''};
}
