...
def build_execution_plan(
    ...
    def issue(code: str, message: str, *, table: str = "", severity: str = "error") -> None:
        issues.append({"code": code, "message": message, "table": table, "severity": severity})

    if replacing:
        if not sqlite:
            issue("replacement_not_supported", "当前仅验证了 SQLite 的整次事务清空与回滚；此连接暂只支持追加生成。")
        ...
        if execution["reset_identity"] and not reset_supported:
            issue("identity_reset_not_supported", "所选表没有可重置的 SQLite AUTOINCREMENT 序列，请关闭重置 ID。")
    plan: dict[str, Any] = {
