...
继承 [包指南](../../AGENTS.md)；页面与交互见 [static/AGENTS.md](static/AGENTS.md)，测试入口见 [测试指南](../../tests/AGENTS.md)。正式工作台使用 `/api/workbench`；旧 `/api` 的兼容约束不要求保留历史页面导航。
...
- `_application.py` 的业务 API 拒绝跨 Origin、重复 Origin/Host 与 cross-site 请求；默认 loopback supervised worker 还验证 loopback Host，避免 DNS rebinding。外部 ASGI 部署的 Host/proxy 策略由其宿主负责，不能误称所有部署都自动只允许 loopback。
- 保留页面/静态资源的 `no-cache`、防嵌入响应头，以及管理接口的独立 Origin/token/loopback 检查；业务同源检查不能替代组件管理授权。请求校验和异常响应不得回显密钥或含凭据的连接地址。
...
- `ai_settings.py` 持久化普通 AI 设置；`settings_environment.py` 只读汇总当前解释器的环境。页面 handoff、检测草稿与迟到响应规则见前端指南。
...
从仓库根运行 `pytest plugins/sqlseed-web/tests/ -q`；模块导航与隔离规则见 [测试指南](../../tests/AGENTS.md)。并发、schema 约束和配置往返使用真实 SQLite；外部 HTTP/模型边界可替换。修改身份与管理生命周期时保留失败释放、错误脱敏和重启恢复回归。
