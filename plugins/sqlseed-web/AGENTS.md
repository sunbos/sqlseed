...
Web UI for sqlseed: a FastAPI backend wrapping `DataOrchestrator` + the sqlseed-ai self-healing subsystem, plus a dependency-free static frontend (vanilla ES modules, NO build toolchain, NO node\_modules). Navicat-style information architecture (redesigned 2026-08-30 from 5 Navicat Data Generator reference screenshots): a three-step generation wizard (target → object tree + per-column property panel → topo-ordered preview/fill) and a three-pane data browser, plus AI 分析与修复 (formerly 自愈实验室) and the acceptance cockpit (系统面板) as standalone entries. Serves two purposes:
...
│           ├── labels.js        # generator + param Chinese labels (PARAM_LABELS) + Navicat 7-class grouping (GEN_CATEGORIES) + tree column annotations (外键/序列/语义)
│           ├── tree.js          # checkable table/column tree (wizard step2 left pane)
│           ├── genform.js       # column property panel (7-segment Navicat layout): grouped generator dropdown + dynamic params + debounced auto preview + clipped common area (NULL%/unique)
...
* **genform is a 7-segment Navicat panel**: 字段名+类型副标题 → 生成器下拉 → 参数区 → 预览+刷新 → 通用区(NULL%/唯一) → 重置属性. Segment order matters — Navicat puts 预览 **above** 通用区 and 重置属性 at the panel bottom, not next to 刷新. The panel is rebuilt by `render()`; only `paramsHolder` is re-cleared in place when the generator changes, so never relocate the params block by `querySelector('.genform-section')` (the old code did, and it broke as soon as a second section existed).

* **Common area is clipped per generator** (`NO_COMMON_GENS` / `NO_UNIQUE_GENS` / `NO_PREVIEW_GENS` in genform.js, per `navicat_generator_ui_reference.md` §1.3): 词表类(枚举/文本/图像或二进制) hides 设置唯一, 图像或二进制(`bytes`) also hides 预览. `NO_COMMON_GENS` is currently **empty** — Navicat's 序列 maps to sqlseed's `skip` (autoincrement, generated by the DB, not offered in the dropdown), NOT to `template`; `template` can contain random fragments (`SKU-{random_string:4}-{sequence:03d}`), so NULL%/unique are meaningful for it. Clipping `template` once made those two invisible and unfixable. `buildCfg()` (shared by `emit()` and `doPreview()`) re-applies the same clipping when building the ColumnConfig — otherwise switching to a clipped generator silently keeps sending an invisible NULL%/unique.
...
* **Date bounds are backfilled when absent**: LLMs routinely emit `datetime` with **no params at all** (verified with gemma4:31b-cloud — every date column came back `{}` while float/choice/template got params). The panel would then look empty even though the core was silently using `start_year=2000`. `applyDateDefaults()` fills `2000-01-01` / `<current-year>-12-31` **only when no bound of any kind is set**, so a partial AI config is never overwritten. Navicat's 日期 panel pre-fills its bounds too.
...
* **Generator dropdown uses Navicat's 7 classes** (通用/个人/支付/商业/位置/产品/电脑, `GEN_CATEGORIES` in labels.js). 支付 and 产品 have no sqlseed generators yet — `groupGenerators()` marks them `pending:true` and the dropdown renders them as disabled placeholders; they become selectable automatically once the P2 generators land, with no change here.

* **Navicat parity spec**: `docs/superpowers/plans/navicat_generator_parity.md` records the per-generator config granularity from the reference screenshots (示例UI/生成数据类型/) and the P0/P1/P2 backlog — consult it before extending generator params. Per-screenshot UI baseline (layout, controls, defaults, options for all 44 generators) lives in `docs/superpowers/plans/navicat_generator_ui_reference.md` — check against it when verifying the property panel.
