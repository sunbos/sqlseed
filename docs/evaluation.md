...
`ColumnMapper` 的 9 级策略链（68 条精确规则 + DEFAULT 检查 + 25 条模式正则 + NULLABLE 回退 + 22 种类型回退）在同类工具中罕见。这意味着 sqlseed 即使在零配置模式下，也能为大多数常见列名和类型选择合理的生成器——这是"开箱即用"体验的基础。
...
- `test_core/` 目录已包含 `test_expression.py`、`test_column_dag.py`、`test_constraints.py`、`test_transform.py`、`test_enrichment.py`、`test_unique_adjuster.py`、`test_plugin_mediator.py`
