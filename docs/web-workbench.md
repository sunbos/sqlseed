...
预览表头第二行展示真实字段类型及 PK、FK、NULL 等关键信息。列名直接打开字段信息，“规则：…”直接打开取值规则，两者在各自悬停时显示下划线；同一时刻只打开一个字段面板。规则面板也可进入 AI 助手，针对该列提出建议。字段信息是只读结构，规则编辑不修改数据库 schema。批量预览优先按后端执行顺序排列标签，保留无法排入顺序的错误表；浏览顺序不改变生成范围。
