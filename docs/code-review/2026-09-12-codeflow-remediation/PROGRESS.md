...
| 检查 | 原始基线 | 第一轮推送 `064854c` | 第二轮推送 `ef7b279` |
| --- | --- | --- | --- |
| CodeFlow 远端总数 | 45 errors + 1987 warnings | 338 errors + 531 warnings | **0 errors + 101 warnings** |
| ESLint 8.57.1 | 37 个 error，具体文本不可读 | 0 errors / 0 warnings | 0 errors / 0 warnings |
| Pylint 2.17.7 | 8 errors + 1837 warnings | 338 errors + 511 warnings | 0 errors，81 条提示 |
| jscpd 4.0.5 | 75 组 / 150 条 | 10 组 / 20 条 | 10 组 / 20 条 |
...
第三轮全仓本地检查为 **0 errors、74 条 Pylint 提示**，jscpd 为 **7 组、75 行、0.09%**，待推送复验。其余 74 条分别是 51 条顶层/可选能力异常边界、11 条精确类型合同、10 条保留不可哈希输入语义的成员判断、2 条手工资源所有权提示；逐项位置和理由见 [最新清单](third-pass/retained-findings-current.json)。剩余 clone 涉及明确的 generator 签名、两阶段流式状态传递和运行时/类型声明的组件依赖，未用动态签名或单用途包装层换取计数下降。
...
第一轮 `064854c9c90a9b890e58a314fbe5d8fba69d1508` 和第二轮 `ef7b2795370873c915b9d1c9b50b59c6ac7016fb` 均已推送并采集完整 CodeFlow 结果。远端 errors 已清零，warnings 尚未清零。
