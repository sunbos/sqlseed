...
本目录只定义 hook contract 与插件生命周期，承接[核心包规则](../AGENTS.md)；具体 CLI、AI、MCP、Web 实现在仓库 `plugins/`，通用运行时 mediation 在 [../core/plugin_mediator.py](../core/plugin_mediator.py)。
...
- `sqlseed_before_generate` / `sqlseed_after_generate` 围绕生成；`sqlseed_before_insert` / `sqlseed_after_insert` 围绕 batch 写入；`sqlseed_shared_pool_loaded` 在 shared pool 注册后调用。`after_generate` 不在 finally 中，也早于 self-ref 后处理，不能用它表示所有后处理成功或保证失败时总被调用。
- template pool 仅用于未由用户配置、非 UNIQUE、无 default、非 PK/自增的 string 列；非空 hook 返回值被改成内部 `foreign_key` spec 抽样。更改 eligible 条件时同时核对 mediator 的真实 spec 结果。
...
- hookspec 修改同步 [docs/guide.md](../../../docs/guide.md#plugin-system)、[CLAUDE.md](../../../CLAUDE.md)、[docs/architecture.md](../../../docs/architecture.md) 与[中文版架构](../../../docs/architecture.zh-CN.md)的 hook 参考；本文件的返回值列表也需核对。
