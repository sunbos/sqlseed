...
USER_ROWS_SQL = "SELECT email, name, age FROM users ORDER BY user_id"
...
def s1_seed_reproducibility() -> None:
    """Compare stored rows from two independently seeded fills."""
    ...
    rows1 = sqlite3.connect(db1).execute(USER_ROWS_SQL).fetchall()
    rows2 = sqlite3.connect(db2).execute(USER_ROWS_SQL).fetchall()
    check(
        "S1 same seed -> identical rows",
        rows1 == rows2 and len(rows1) == 100,
    ...
    rows3 = sqlite3.connect(db3).execute(USER_ROWS_SQL).fetchall()
    check("S1 different seed -> different rows", rows1 != rows3)


def s2_provider_compatibility() -> None:
    """Verify Base and Faker providers generate valid stored rows."""
    ...
    for provider in ("base", "faker"):
        ...
        try:
            ...
            con.close()
            check(f"S2 provider={provider} fills 50 unique-email rows", n == 50 and dup == 0, f"n={n} dup={dup}")
...
def s3_cli_surface() -> None:
    """Exercise fill, preview, and inspect with the Click runner."""
    print("\n[S3] CLI surface")
    ...
    check("S3 cli inspect", r.exit_code == 0 and "users" in r.output, f"exit={r.exit_code} out={r.output[-200:]}")


def s4_boundary_counts() -> None:
    """Verify zero and single-row fill requests."""
    print("\n[S4] boundary counts")
    import sqlseed
    ...
    try:
        ...
        n1 = sqlite3.connect(db).execute(COUNT_USERS_SQL).fetchone()[0]
...
def s5_large_scale() -> None:
    """Check uniqueness and foreign keys across a larger generated dataset."""
    print("\n[S5] large-scale fill")
    import sqlseed
    ...
    dup = con.execute("SELECT COUNT(*) - COUNT(DISTINCT email) FROM users").fetchone()[0]
    orphans = con.execute("SELECT COUNT(*) FROM orders WHERE user_id NOT IN (SELECT user_id FROM users)").fetchone()[0]
    con.close()
    check(
        "S5 2k users + 20k orders, unique emails, no FK orphans",
...
def s6_preview_isolation() -> None:
    """Verify preview leaves the persistent database unchanged."""
    print("\n[S6] preview isolation")
    import sqlseed

    db = build_db("s6.db", SHOP_DDL)
    rows = sqlseed.preview(str(db), table="users", count=10, seed=3)
    n = sqlite3.connect(db).execute(COUNT_USERS_SQL).fetchone()[0]
    check(
        "S6 preview returns rows without writing", len(rows) == 10 and n == 0, f"preview_rows={len(rows)} db_rows={n}"
    )


def s7_yaml_round_trip() -> None:
    """Verify YAML configuration preserves seed and derived values."""
    print("\n[S7] YAML round-trip")
    import yaml
    ...
    con.close()
    check(
        "S7 derive_from expression + CHECK compliance",
...
def s8_error_handling() -> None:
    """Verify invalid tables and configurations report failures."""
    print("\n[S8] error handling")
    import sqlseed
    ...
    check(
        "S8 unknown table surfaces error in result",
        r.count == 0 and bool(r.errors),
        f"count={r.count} errors={r.errors}",
    )
...
def main() -> int:
    """Run the eight smoke scenarios and return their combined status."""
    print("=" * 70)
