...
class TestGeneratorTypes:
    ...
    def test_new_types_documented(self):
        ...
        for gen in sorted(generators):
            ...
            assert gen in readme, (
                f"Generator '{gen}' is in _GENERATOR_MAP but not mentioned in README.md"
            )
...
class TestExactMatchRules:
    ...
    def test_count_in_docs(self):
        ...
        rule_section = re.search(
            r"EXACT_MATCH_RULES.*?=.*?\{(.*?)\}", code, re.DOTALL
        )
        if not rule_section:
        ...
        for doc_path in _find_doc_files():
            ...
            for m in matches:
                assert int(m) == rule_count, (
                    f"{doc_path}: claims {m} rules, but EXACT_MATCH_RULES has {rule_count}"
                )
        ...
        for doc_path in _find_doc_files():
            ...
            for m in matches:
                ...
                if "expression" in text.lower() or "表达式" in text.lower() or "SAFE_FUNCTIONS" in text:
                    assert int(m) == count, (
                        f"{doc_path}: claims {m} expression functions, "
                        f"but SAFE_FUNCTIONS has {count}"
                    )
...
class TestPluginHooks:
    ...
    def test_count_in_docs(self):
        ...
        for doc_path in _find_doc_files():
            ...
            for m in matches:
                assert int(m) == count, (
                    f"{doc_path}: claims {m} hooks, but hookspecs.py has {count}"
                )
...
class TestCodeExamples:
    ...
    def test_public_api_in_readme(self):
        ...
        for func in public_funcs:
            assert func in readme, (
                f"Public API function '{func}' not mentioned in README.md"
            )
