...
def _make_config(tmp_path, db_path: str, **overrides) -> str:
    config_data = {
        "db_path": db_path,
        "provider": "base",
        "locale": "en_US",
...
class TestCLIOverridesYAMLCount:
    def test_yaml_count_no_cli_override(self, tmp_path) -> None:
    ...
    def test_cli_count_overrides_yaml(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
    ...
    def test_cli_count_overrides_yaml_large(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
...
class TestCLIOverridesYAMLClearBefore:
    def test_no_clear_appends_data(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
    ...
    def test_clear_before_resets_data(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
        sqlseed.fill_from_config(config_path, skip_ai=True, count=10)
    ...
    def test_clear_before_resets_autoincrement_id(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
        sqlseed.fill_from_config(config_path, skip_ai=True, count=10)
    ...
    def test_yaml_clear_before_true_no_cli(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(
            tmp_path, db_path,
    ...
    def test_yaml_clear_false_cli_clear_true(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
        sqlseed.fill_from_config(config_path, skip_ai=True, count=10)
        sqlseed.fill_from_config(config_path, skip_ai=True, count=5, clear_before=True)
    ...
    def test_no_clear_id_continues(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
        sqlseed.fill_from_config(config_path, skip_ai=True, count=10)
        sqlseed.fill_from_config(config_path, skip_ai=True, count=5)
...
class TestCLIOverridesYAMLSeed:
    def test_cli_seed_overrides_yaml(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(
            tmp_path, db_path,
        ...
        results_a = sqlseed.fill_from_config(
            config_path, skip_ai=True, seed=42, clear_before=True,
        )
    ...
    def test_different_seeds_different_data(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
...
class TestCLIOverridesYAMLBatchSize:
    def test_cli_batch_size_overrides_yaml(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(
            tmp_path, db_path,
...
class TestCLIOverridesYAMLProvider:
    def test_cli_provider_overrides_yaml(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path, provider="base")
        results = sqlseed.fill_from_config(
            config_path, skip_ai=True, provider="mimesis",
        )
        assert results[0].count == 100
        assert _count_rows(db_path) == 100
...
class TestCLIOverridesYAMLLocale:
    def test_cli_locale_overrides_yaml(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
...
class TestAgeNoHardConstraint:
    def test_age_respects_yaml_max_100(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(
            tmp_path, db_path,
            tables=[{
    ...
    def test_age_respects_yaml_max_65(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(
            tmp_path, db_path,
            tables=[{
...
class TestCLIConfigPriorityIntegration:
    def test_count_and_clear_combined(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
    ...
    def test_all_overrides_combined(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(
            tmp_path, db_path,
            tables=[{
                "name": "users", "count": 100, "seed": 1,
                "batch_size": 5000, "columns": [],
            }],
        )
        results = sqlseed.fill_from_config(
            config_path,
    ...
    def test_cli_fill_config_with_count_override(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
        runner = CliRunner()
        result = runner.invoke(
            cli, ["fill", "--config", config_path, "--count", "5", "--no-ai"],
        )
        assert result.exit_code == 0
        assert _count_rows(db_path) == 5

    def test_cli_fill_config_with_clear(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
        runner = CliRunner()
    ...
    def test_cli_fill_config_note_when_no_clear(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
        runner = CliRunner()
        result = runner.invoke(
            cli, ["fill", "--config", config_path, "--no-ai"],
        )
        assert "Data will be appended" in result.output

    def test_cli_fill_config_no_note_when_clear(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(tmp_path, db_path)
        runner = CliRunner()
        result = runner.invoke(
            cli, ["fill", "--config", config_path, "--clear", "--no-ai"],
        )
        assert "Data will be appended" not in result.output

    def test_cli_fill_config_no_note_when_yaml_clear(self, tmp_path) -> None:
        db_path = _make_db(tmp_path)
        config_path = _make_config(
            tmp_path, db_path,
            tables=[{"name": "users", "count": 10, "clear_before": True, "columns": []}],
