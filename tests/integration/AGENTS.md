...
- `pg_url` fixture 用 testcontainers 启动 `postgres:16-alpine`，把连接 scheme 调整成 `postgresql+psycopg`；需要 Docker daemon、testcontainers 和 psycopg。
...
```bash
pytest tests/integration/test_pg_integration.py tests/integration/test_url_e2e.py -v
