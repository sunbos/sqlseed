...
| 日志、metrics、paths、progress、后台任务 | [test_utils/AGENTS.md](test_utils/AGENTS.md) |
...
- CLI 单元测试使用 `click.testing.CliRunner`；发行包验收脚本需启动真实入口以验证安装结果，不能由 CliRunner 替代。AI 可选依赖用 `pytest.importorskip("sqlseed_ai")` 或现有模块级 skip 模式。
...
- 根 `pyproject.toml` 的默认 `pytest` 包含 tests、CLI、AI、MCP、Web；`make test-core` 不包含本目录根层用例及 `test_utils/`。
...
- Windows/macOS 兼容性 CI 将 `ResourceWarning` 与 `PytestUnraisableExceptionWarning` 视为错误；连接和后台任务必须有明确的释放边界，不以全局 warning filter 掩盖泄漏。
