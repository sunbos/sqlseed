...
def test_transaction_queries_and_insert_calls_share_uncommitted_rows(
...
def test_late_batch_failure_rolls_back_deletes_rows_and_sequence_reset(
    transaction_database: tuple[Path, SQLAlchemyAdapter],
) -> None:
    path, adapter = transaction_database
...
def test_nested_transaction_is_rejected_and_outer_transaction_can_rollback(
    transaction_database: tuple[Path, SQLAlchemyAdapter],
) -> None:
...
def test_deferred_foreign_key_failure_on_commit_restores_original_data(
    transaction_database: tuple[Path, SQLAlchemyAdapter],
) -> None:
    path, adapter = transaction_database
