from __future__ import annotations

import sqlite3
from typing import TYPE_CHECKING

import pytest

from sqlseed.database.sqlite_utils_adapter import SQLiteUtilsAdapter

if TYPE_CHECKING:
    from pathlib import Path


@pytest.fixture
def db_path(tmp_path: Path) -> str:
    path = str(tmp_path / "test.db")
    conn = sqlite3.connect(path)
    conn.execute(
        "CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, "
        "name TEXT NOT NULL, email TEXT, age INTEGER, active INTEGER DEFAULT 1)"
    )
    conn.execute(
        "CREATE TABLE orders (id INTEGER PRIMARY KEY AUTOINCREMENT, "
        "user_id INTEGER REFERENCES users(id), amount REAL, created_at TEXT)"
    )
    conn.commit()
    conn.close()
    return path


class TestSQLiteUtilsAdapter:
    def test_connect_and_close(self, db_path: str) -> None:
        adapter = SQLiteUtilsAdapter()
        adapter.connect(db_path)
        assert adapter._db is not None
        adapter.close()
        assert adapter._db is None

    def test_get_table_names(self, db_path: str) -> None:
        adapter = SQLiteUtilsAdapter()
        adapter.connect(db_path)
        tables = adapter.get_table_names()
        assert "users" in tables
        assert "orders" in tables
        adapter.close()

    def test_get_column_info(self, db_path: str) -> None:
        adapter = SQLiteUtilsAdapter()
        adapter.connect(db_path)
        columns = adapter.get_column_info("users")
        col_names = [c.name for c in columns]
        assert "id" in col_names
        assert "name" in col_names
        assert "email" in col_names
        assert "age" in col_names
        assert "active" in col_names

        id_col = next(c for c in columns if c.name == "id")
        assert id_col.is_primary_key is True
        assert id_col.is_autoincrement is True

        name_col = next(c for c in columns if c.name == "name")
        assert name_col.type is not None
        adapter.close()

    def test_get_primary_keys(self, db_path: str) -> None:
        adapter = SQLiteUtilsAdapter()
        adapter.connect(db_path)
        pks = adapter.get_primary_keys("users")
        assert "id" in pks
        adapter.close()

    def test_get_foreign_keys(self, db_path: str) -> None:
        adapter = SQLiteUtilsAdapter()
        adapter.connect(db_path)
        fks = adapter.get_foreign_keys("orders")
        assert len(fks) >= 1
        fk = fks[0]
        assert fk.column == "user_id"
        assert fk.ref_table == "users"
        assert fk.ref_column == "id"
        adapter.close()

    def test_batch_insert(self, db_path: str) -> None:
        adapter = SQLiteUtilsAdapter()
        adapter.connect(db_path)
        data = [
            {"name": "Alice", "email": "alice@test.com", "age": 30, "active": 1},
            {"name": "Bob", "email": "bob@test.com", "age": 25, "active": 1},
        ]
        inserted = adapter.batch_insert("users", iter(data), batch_size=100)
        assert inserted == 2
        count = adapter.get_row_count("users")
        assert count == 2
        adapter.close()

    def test_batch_insert_large(self, db_path: str) -> None:
        adapter = SQLiteUtilsAdapter()
        adapter.connect(db_path)
        data = [{"name": f"User{i}", "email": f"user{i}@test.com", "age": 20 + i % 50, "active": 1} for i in range(100)]
        inserted = adapter.batch_insert("users", iter(data), batch_size=30)
        assert inserted == 100
        adapter.close()

    def test_clear_table(self, db_path: str) -> None:
        adapter = SQLiteUtilsAdapter()
        adapter.connect(db_path)
        data = [{"name": "Alice", "email": "a@t.com", "age": 30, "active": 1}]
        adapter.batch_insert("users", iter(data))
        assert adapter.get_row_count("users") == 1
        adapter.clear_table("users")
        assert adapter.get_row_count("users") == 0
        adapter.close()

    def test_get_column_values(self, db_path: str) -> None:
        adapter = SQLiteUtilsAdapter()
        adapter.connect(db_path)
        data = [
            {"name": "Alice", "email": "alice@test.com", "age": 30, "active": 1},
            {"name": "Bob", "email": "bob@test.com", "age": 25, "active": 1},
        ]
        adapter.batch_insert("users", iter(data))
        names = adapter.get_column_values("users", "name")
        assert "Alice" in names
        assert "Bob" in names
        adapter.close()

    def test_context_manager(self, db_path: str) -> None:
        with SQLiteUtilsAdapter() as adapter:
            adapter.connect(db_path)
            tables = adapter.get_table_names()
            assert len(tables) >= 1

    def test_optimize_and_restore(self, db_path: str) -> None:
        adapter = SQLiteUtilsAdapter()
        adapter.connect(db_path)
        adapter.optimize_for_bulk_write(50000)
        adapter.restore_settings()
        adapter.close()

    def test_get_row_count(self, db_path: str) -> None:
        adapter = SQLiteUtilsAdapter()
        adapter.connect(db_path)
        assert adapter.get_row_count("users") == 0
        data = [{"name": "A", "email": "a@t.com", "age": 20, "active": 1}]
        adapter.batch_insert("users", iter(data))
        assert adapter.get_row_count("users") == 1
        adapter.close()
