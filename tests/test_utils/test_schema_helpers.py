"""Tests for ``sqlseed._utils.schema_helpers.detect_autoincrement``.
...
from sqlseed._utils.schema_helpers import _split_sql_definitions, detect_autoincrement
...
# ---------------------------------------------------------------------------
# detect_autoincrement tests
# ---------------------------------------------------------------------------


class TestDetectAutoincrement:
    def test_basic_autoincrement(self) -> None:
        ddl = "CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT)"
        assert detect_autoincrement(_make_execute_fn(ddl), "users", "id") is True
    ...
    def test_decimal_column_with_autoincrement(self) -> None:
        ...
        assert detect_autoincrement(_make_execute_fn(ddl), "products", "id") is True
    ...
    def test_check_constraint_with_autoincrement(self) -> None:
        ...
        assert detect_autoincrement(_make_execute_fn(ddl), "orders", "id") is True

    def test_lowercase_autoincrement(self) -> None:
        ddl = "CREATE TABLE t (id integer primary key autoincrement)"
        assert detect_autoincrement(_make_execute_fn(ddl), "t", "id") is True
    ...
    def test_no_autoincrement_returns_false(self) -> None:
        ...
        assert detect_autoincrement(_make_execute_fn(ddl), "t", "id") is False
    ...
    def test_autoincrement_on_other_column_returns_false_for_target(self) -> None:
        ...
        assert detect_autoincrement(_make_execute_fn(ddl), "t", "a") is False
        assert detect_autoincrement(_make_execute_fn(ddl), "t", "b") is True
    ...
    def test_column_name_not_present(self) -> None:
        ...
        assert detect_autoincrement(_make_execute_fn(ddl), "t", "name") is False

    def test_no_ddl_returns_false(self) -> None:
        assert detect_autoincrement(_make_execute_fn(None), "t", "id") is False

    def test_execute_fn_raises_returns_false(self) -> None:
        mock = MagicMock(side_effect=sqlite3.Error("connection closed"))
        assert detect_autoincrement(mock, "t", "id") is False
    ...
    def test_column_name_word_boundary(self) -> None:
        ...
        assert detect_autoincrement(_make_execute_fn(ddl), "t", "auto") is False
...
class TestDetectAutoincrementIntegration:
    ...
    def test_real_autoincrement(self, conn: sqlite3.Connection) -> None:
        ...
        assert detect_autoincrement(conn.execute, "t", "id") is True

    def test_real_no_autoincrement(self, conn: sqlite3.Connection) -> None:
        conn.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT)")
        assert detect_autoincrement(conn.execute, "t", "id") is False
